...
const ClassSheduleSeccion1 = () => {
    ...
    const [optionsGruop, setOptionsGroup] = useState([]);
    const [optionsTutor, setOptionsTutor] = useState([]);
    ...
    const getClassShedule = async () => {
        if (selectOptionPeriod && selectOptionSemester && selectOptionCarrer) {
            try {
                const response = await api.get(`${urlGlobal}/class-schedules/by-datas`, {
                    headers: {
                        Authorization: `Bearer ${userStore.tokens?.access_token}`,
                    },
                    params: {
                        career_id: selectOptionCarrer.id,
                        semester: selectOptionSemester,
                        period_id: selectOptionPeriod.id,
                    },
                });
    ...
    const getOptionsTutor = async () => {
        try {
            ...
            setOptionsTutor(response.data.data.teachers);
        } catch (error) {
            console.error("Error fetching personal data:", error);
        }
    }
    ...
    const importClassShedule = () => {
        importRef.current.click();
    }

    const getOptions = async () => {
        await getGroupOptions();
        await getOptionsTutor();
        ...
        <div className='w-full pb-4'>
            ...
            <div className='w-full flex items-center justify-start min-[321px]:justify-end mt-2 gap-1'>
                <div className='min-[321px]:hidden flex gap-1 items-center'>
                    <SelectInput notSelectDefault={true} className={"min-[321px]:hidden md:w-3/12 md:h-9"} setOption={getClassShedule} setValue={setSelectOptionPeriod} options={optionsPeriod} title='Periodo' titleEnter={false}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                        </svg>
                    </SelectInput>
                    <SelectInput notSelectDefault={true} className={"min-[321px]:hidden md:w-3/12 md:h-9"} setOption={getClassShedule} setValue={setSelectOptionCarrer} options={optionsCarrer} title='Carrera' titleEnter={false}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5" />
                        </svg>
                    </SelectInput>
                    <SelectInput notSelectDefault={true} className={"min-[321px]:hidden md:w-3/12 md:h-9"} setOption={getClassShedule} setValue={setSelectOptionSemester} options={optionsSemester} title='Semestre' titleEnter={false}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                ...
                <div className='w-full flex gap-4 justify-end'>
                    <Button onClick={importClassShedule} className={'pr-0 text-sm md:text-base'}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                    ...
                    <input accept=".xlsx,.xls,.csv" ref={importRef} className='hidden' type="file" />
                    <Button onClick={getClassShedule} className={"w-auto px-1.5 py-2 text-sm md:text-base md:w-24 ring-1 ring-cyan-600 bg-cyan-600 text-white rounded md:px-4 hover:ring-3"}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
            ...
            <div className='hidden min-[370px]:visible min-[370px]:flex items-center justify-start gap-2 md:gap-20 xl:gap-28 mt-5'>
                <SelectInput valueOption='period_code' notSelectDefault={true} className={"min-[800px]:w-2/12 min-[700px]:w-36 md:h-9"} titleMovil={"Seleccionar periodo"} setOption={getClassShedule} setValue={setSelectOptionPeriod} options={optionsPeriod} title='Periodo' titleEnter={false}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                    </svg>
                </SelectInput>
                <SelectInput valueOption='career_name' notSelectDefault={true} className={"min-[800px]:w-44 min-[700px]:w-36 md:h-9"} titleMovil={"Seleccionar carrera"} setOption={getClassShedule} setValue={setSelectOptionCarrer} options={optionsCarrer} title='Carrera' titleEnter={false}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5" />
                    </svg>
                </SelectInput>
                <SelectInput valueOption='semester' notSelectDefault={true} className={"md:w-40 md:h-9"} titleMovil={"Seleccionar semestre"} setOption={getClassShedule} setValue={setSelectOptionSemester} options={optionsSemester} title='Semestre' titleEnter={false}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8.242 5.992h12m-12 6.003H20.24m-12 5.999h12M4.117 7.495v-3.75H2.99m1.125 3.75H2.99m1.125 0H5.24m-1.92 2.577a1.125 1.125 0 1 1 1.591 1.59l-1.83 1.83h2.16M2.99 15.745h1.125a1.125 1.125 0 0 1 0 2.25H3.74m0-.002h.375a1.125 1.125 0 0 1 0 2.25H2.99" />
                    </svg>
                </SelectInput>
            ...
            {infoCell.length > 0 &&
                <>
                    ...
                    < TableShedule setBottonEdit={openModalTableEdit} footerValue={footerValue} headValue={headValue} valueCell={infoCell}></TableShedule>
                </>
            }

            <Modal show={showTableModal} onDisable={closeModalTable} aceptModal={false} fullScreen={showTable}>
                <div className='w-full px-2 py-4'>
                    <h3 className='text-center text-md md:text-xl font-semibold mb-3'>{tableAccion.new ? "Nuevo horario" : "Editar tabla"}</h3>
                    <TableShedule setFooterValue={setFooterValue} setShowModal={setShowTable} options={{ carrer: optionsCarrer, smt: optionsSemester, period: optionsPeriod, grp: optionsGruop, tutor: optionsTutor }} setHeadValue={setHeadValue} closeTable={closeModalTable} saveTable={saveTable} edit={true} updateTable={tableAccion.update} newTable={tableAccion.new} headValue={headValue} valueCell={infoCell} setValueCell={setInfoCell}></TableShedule>
                </div>
            </Modal>
        </div >
    )
...
export default ClassSheduleSeccion1
