...
function Table({ Heads = [],  textModal = "¿ Estas seguro de eliminar este elemento ?" ,optionsPersonal = [], dates = [], datesCard = ["correo","status"], showModalDelete, indexDelete, deleteAprob, deletePersonal, closeModalDelete, showDelete }) {
    return (
        <div className='w-full mt-6 md:overflow-hidden'>
                ...
                optionsPersonal.length > 0
                    ...
                    <>
                        ...
                        <div className='flex flex-col visible md:hidden'>
                            {
                                optionsPersonal.map((option) => (
                                    <CardPersonal info={[datesCard[0], datesCard[1]]} key={option.id} item={option} index={option.id} onClickDelete={showModalDelete} indexDelete={indexDelete} deleteAprob={deleteAprob} />
                                ))
                            }
                        </div>
                    </>
            ...
            <Modal  onClickAccept={deletePersonal} show={showDelete} onDisable={closeModalDelete} text={textModal} />
        </div>
    )
}
