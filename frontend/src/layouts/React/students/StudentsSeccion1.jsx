...
function StudentsSeccion1() {
  ...
  const getPersonal = async () => {
    // try {
    //     const response = await fetch(`/api/personal?search=${searchStudents}`);
    //     if (!response.ok) {
      ...
      <div>
        <div className="w-full flex justify-between mt-4">
          <div className="flex md:gap-2 justify-start gap-0.5 w-9/12">
            ...
            <InputSearch valueSearch={"nombre"} className={"md:w-full md:h-11"} getOptions={getPersonal} options={valuesStudents} value={searchStudents} setValue={setSearchStudents} title="Buscar personal" />
          </div>
        </div>

        <Table Heads={headNames} values={valuesStudents} dates={dates} showModalDelete={showModalDelete} indexDelete={indexDelete} deleteAprob={deleteAprob} deleteValue={deletePersonal} closeModalDelete={closeModalDelete} showDelete={showDelete} />
      </div>
    </>
  )
}
